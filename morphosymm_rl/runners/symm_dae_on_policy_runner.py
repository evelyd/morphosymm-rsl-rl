...
from morphosymm_rl.modules import ActorCriticSymm
...
class SymmDAEOnPolicyRunner:
    ...
    def load(self, path: str, load_optimizer: bool = True, map_location: str | None = None) -> dict:
        ...
        if "dae" in self.task and "dae_state_dict" in loaded_dict:
            ...
            # If using the equivariant DAE, we MUST strip the escnn caches to prevent loading crashes
            if "ecdae" in self.task:
                dae_state = {
                    key: value for key, value in dae_state.items()
                    if not key.endswith(".matrix") and not key.endswith(".expanded_bias")
                }
                # Use strict=False because we deliberately removed the cache keys
    ...
    def _construct_algorithm(self, obs: TensorDict):
        ...
        # Initialize Symmetric Actor Critic
        ...
        # Standard RSL-RL storage hack
        storage = RolloutStorage("rl", self.env.num_envs, self.cfg["num_steps_per_env"], obs, [self.env.num_actions], self.device)
