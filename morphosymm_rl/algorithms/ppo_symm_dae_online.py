...
# Import your helper functions from PPOSymmDataAugmented here
...
class PPOSymmDAEOnline:
    def __init__(
        ...
        **morphologycal_symmetries_cfg,
        ...
        self.replay_buffer = PrioritizedReplayBuffer(self.state_dim, self.action_dim, koopman_cfg["beta_initial"], koopman_cfg["beta_annealing_steps"], koopman_cfg["replay_buffer_size"])
        self.obs_action_normalizer = RunningStdScaler(self.state_dim, self.action_dim, device=self.device)

        # Initialize the ECDAE model passing the group G from the symmetric policy
        self.dae_model = initialize_dae_model(
            morphologycal_symmetries_cfg=morphologycal_symmetries_cfg,
            koopman_cfg=koopman_cfg,
            G=self.G,  # Passing the group
            task=self.task,
        ...
        # Setup Expanded Storage for Symmetries
    ...
    def dae_predict(self, obs, action):
        ...
        dae_input = dae_input.to(dtype=next(self.dae_model.parameters()).dtype, device=next(self.dae_model.parameters()).device)
        dae_input_normed, action_normed = self.obs_action_normalizer.normalize(dae_input, action)
        action_normed = action_normed.unsqueeze(1).repeat(1, 1, 1)
        _, next_latents = self.dae_model.forecast(dae_input_normed, action_normed, n_steps=1)
        return next_latents[:, -1, :].detach()
    ...
    def act(self, obs: TensorDict) -> torch.Tensor:
        ...
        # Inject the Koopman prediction *before* evaluation so the critic sees it
    ...
    def process_env_step(self, obs: TensorDict, rewards: torch.Tensor, dones: torch.Tensor, extras: dict[str, torch.Tensor]) -> None:
        ...
        # Augment the transitions symmetrically before pushing to storage
    ...
    def augment_transitions(self):
        ...
        # 2. Augment Policy Obs
        # (Policy obs is just one group, so we can transform it directly)
        ...
        # 3. Augment Critic Obs & Koopman Prediction
        # The symmetry field expects the FULL concatenated 519-dimensional tensor
        full_critic_obs = self.policy.get_critic_obs(t.observations)

        # Transform the concatenated tensor
        transformed_full_critic = [critic_in_field_type.transform_fibers(full_critic_obs, g) for g in G.elements[1:]]

        # Now split it back into the base critic and the koopman components
    ...
    def compute_returns(self, obs: TensorDict, last_action: torch.Tensor) -> None:
        st = self.storage
        obs["koopman_prediction"] = self.dae_predict(obs, last_action)
