...
from morphosymm_rl.storage import ReplayBuffer, RunningStdScaler, PrioritizedReplayBuffer
from morphosymm_rl.modules.dae_actor_critic import DAEActorCritic
...
class PPODAEOnline:
    ...
    policy: ActorCritic | ActorCriticRecurrent | ActorCriticCNN | DAEActorCritic
    ...
    def __init__(
        ...
        self.replay_buffer = PrioritizedReplayBuffer(self.state_dim, self.action_dim, koopman_cfg["beta_initial"], koopman_cfg["beta_annealing_steps"], koopman_cfg["replay_buffer_size"]) #default device will be cpu
        ...
        # Values for getting the correct critic input
    ...
    def dae_predict(self, obs, action):
        ...
        critic_obs = obs["critic"]  # shape: (batch_size, history_length * single_observation_space)
        dae_input = critic_obs[:, self.single_observation_space*(self.history_length-1):self.single_observation_space*self.history_length] # most recent state
        ...
        _, next_latents = self.dae_model.forecast(dae_input_normed, action_normed, n_steps=n_steps) # default forecast horizon H=110000
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            obs_batch["koopman_prediction"] = self.dae_predict(obs_batch, actions_batch)
