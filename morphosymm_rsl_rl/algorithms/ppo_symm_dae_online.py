...
class PPOSymmDAEOnline:
    """Equivariant Proximal Policy Optimization with Online ECDAE integration (rsl_rl v5.4.2 style)."""
    ...
    @classmethod
    def construct_algorithm(cls, obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> PPOSymmDAEOnline:
        ...
        # 1. Resolve escnn symmetry representations
        ...
        # 2. Build Actor and Critic as SymmModels (Intercepting the class name)
        ...
        # Instantiate networks dynamically, injecting **symm_cfg so they remain Equivariant
        ...
        tiled_obs = TensorDict(
            {key: PPOSymmDAEOnline._tile(value, num_replica) for key, value in obs.items()},
        ...
        # Initialize the ECDAE model with the symmetry group G
        alg.dae_model = initialize_dae_model(
            morphologycal_symmetries_cfg=symm_cfg,
            koopman_cfg=koopman_cfg,
            G=G,
            task=task,
        ...
        alg.dae_optimizer = torch.optim.Adam(alg.dae_model.parameters(), lr=koopman_cfg.get("lr", 1e-3))
    ...
    def dae_predict(self, obs: TensorDict, action: torch.Tensor) -> torch.Tensor:
        ...
        dae_input = dae_input.to(
            dtype=next(self.dae_model.parameters()).dtype, device=next(self.dae_model.parameters()).device
        )
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            # Predict next latent state using DAE
